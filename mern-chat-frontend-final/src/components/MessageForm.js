...
function MessageForm() {
    ...
    async function fetchTimeSensitiveMessages() {
        try {
            const response = await fetch("http://localhost:5001/messages/remainders");
            const data = await response.json();
            console.log(data);
            setTimeSensitiveMessages(data.data || []); // Use data.data if the backend wraps in `data`
        } catch (error) {
            console.error(error);
        }
    }
                ...
                <Modal.Body>
                    ...
                    {timeSensitiveMessages.map((msg, idx) => (
                        <div key={idx} className="time-sensitive-message">
                            <p><strong>Message:</strong> {msg.content}</p> {/* Render the content */}
                            {/* If you have additional information, render it here */}
